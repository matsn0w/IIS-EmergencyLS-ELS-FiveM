    ...
    while true do
        ...
        if GetCarHash(GetVehiclePedIsUsing(PlayerPedId())) ~= false then -- If the car player is in exists, then...
            if SetContains(kjxmlData,
                           GetDisplayNameFromVehicleModel(GetEntityModel(
                ...
                SetVehicleRadioEnabled(GetVehiclePedIsUsing(PlayerPedId()),
                                       false)
                SetVehicleAutoRepairDisabled(
                    GetVehiclePedIsUsing(PlayerPedId()), true)
        ...
        if GetLastInputMethod(0) then -- If on keyboard
            ...
            elseif IsDisabledControlJustPressed(1,
                                                Config.KeyBinds['SecondaryLights']) then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    vehicle = GetVehiclePedIsUsing(PlayerPedId())
                    if CanControlSirens(vehicle) then -- If player is driver
                        ...
                        if kjEnabledVehicles[vehicle]["secondary"] then -- If secondary lighting is on, then...
                            TriggerServerEvent("kjELS:updateStatus",
                                               "secondary", false) -- Turn them off
                        else
                            TriggerServerEvent("kjELS:updateStatus",
                                               "secondary", true) -- if not, turn them on!
                        end
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    if CanControlSirens(vehicle) then
                        ...
                        if kjEnabledVehicles[vehicle]["warning"] then
                            TriggerServerEvent("kjELS:updateStatus", "warning",
                                               false)
                        else
                            TriggerServerEvent("kjELS:updateStatus", "warning",
                                               true)
            ...
            elseif IsDisabledControlJustReleased(1,
                                                 Config.KeyBinds['ActivateSiren']) then
                vehicle = GetVehiclePedIsUsing(PlayerPedId())
                if CanControlSirens(vehicle) then
                    if kjEnabledVehicles[vehicle] == nil then
                        AddVehicleToTable(vehicle)
                    ...
                    if kjEnabledVehicles[vehicle].primary == true then
                        if kjEnabledVehicles[vehicle] ~= nil and
                            kjEnabledVehicles[vehicle].siren ~= 0 then
                            TriggerServerEvent("kjELS:setSirenState", 0)
                ...
                if IsControlJustPressed(1, 174) then -- left
                    if IsPedInAnyVehicle(GetPlayerPed(-1), true) then
                        ...
                        if GetPedInVehicleSeat(Veh, -1) == GetPlayerPed(-1) then
                            ...
                            TriggerServerEvent('kjELS:sv_Indicator', 'left',
                                               IndicatorL)
                ...
                if IsControlJustPressed(1, 175) then -- right
                    if IsPedInAnyVehicle(GetPlayerPed(-1), true) then
                        ...
                        if GetPedInVehicleSeat(Veh, -1) == GetPlayerPed(-1) then
                            ...
                            TriggerServerEvent('kjELS:sv_Indicator', 'right',
                                               IndicatorR)
                            PlaySoundFrontend(-1, "NAV_UP_DOWN",
                                              "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
                        end
                ...
                if IsControlJustPressed(1, 173) then -- Hazards
                    if IsPedInAnyVehicle(GetPlayerPed(-1), true) then
                        ...
                        if GetPedInVehicleSeat(Veh, -1) == GetPlayerPed(-1) then
                            ...
                            TriggerServerEvent('kjELS:sv_Indicator', 'hazard',
                                               IndicatorHazard)
                            PlaySoundFrontend(-1, "NAV_UP_DOWN",
                                              "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
                        end
            ...
            if IsDisabledControlJustReleased(1, 85) then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    if CanControlSirens(vehicle) then
                        ...
                        if kjEnabledVehicles[vehicle]["primary"] then
                            TriggerServerEvent("kjELS:updateStatus", "primary",
                                               false)
                            SetVehicleSiren(vehicle, false)
                            TriggerServerEvent("kjELS:setSirenState", 0)
                        else
...
AddEventHandler('updateIndicators', function(PID, dir, toggle)
    local Veh = GetVehiclePedIsIn(GetPlayerPed(GetPlayerFromServerId(PID)),
                                  false)
    if dir == 'left' then
