...
function SLAXML:parse(xml, fileName, options)
    ...
    local find, sub, gsub, char, push, pop, concat = string.find, string.sub,
                                                     string.gsub, string.char,
    ...
    local function findPI()
        first, last, match1, match2 = find(xml,
                                           '^<%?([:%a_][:%w_.-]*) ?(.-)%?>', pos)
        if first then
            finishText()
    ...
    local function nsForPrefix(prefix)
        ...
        for i = #nsStack, 1, -1 do
            if nsStack[i][prefix] then return nsStack[i][prefix] end
        end
            ...
            if self._call.attribute then
                for i = 1, currentAttributeCt do
                    if currentAttributes[i][4] then
                        currentAttributes[i][3] = nsForPrefix(
                                                      currentAttributes[i][4])
                    end
    ...
    local function findElementClose()
        ...
        if first then
            ...
            first, last, match2, match1 = find(xml,
                                               '^</([%a_][%w_.-]*):([%a_][%w_.-]*)%s*>',
                                               pos)
    ...
    while pos < #xml do
        if state == "text" then
            if not (findPI() or findComment() or findCDATA() or
                findElementClose()) then
        ...
        elseif state == "attributes" then
            if not findAttribute() then
                if not closeElement() then
                    print("\n[ERROR] " .. fileName ..
                              " is broken! Please put in online validator and fix!\n")
                    break
                end
            end
        end
    ...
    local builder = SLAXML:parser{
        ...
        attribute = function(name, value, nsURI)
            if not current or current.type ~= "element" then
                error(
                    ("Encountered an attribute %s=%s but I wasn't inside an element"):format(
                        name, value))
            end
            local attr = {
                type = 'attribute',
        ...
        text = function(value)
            if current.type ~= 'document' then
                if current.type ~= "element" then
                    error(
                        ("Received a text notification '%s' but was inside a %s"):format(
                            value, current.type))
                end
                push(current.kids, {
                    type = 'text',
